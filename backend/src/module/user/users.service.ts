...
export class UsersService {
  ...
  async getStatistics(token: string) {
    const hardCodedToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJwcm9qZWN0TmFtZSI6IlRyYWNrZXItMjMiLCJvd25lck5hbWUiOiJTdWRpcHRvIEdob3NoIiwiaWF0IjoxNTE2MjM5MDIyfQ.8xFOck39dRi37Ue8_cM6PLUcFdWsKzcGjZZxKB_j6EI";
    if ( token !== hardCodedToken) {
      throw new APIException('You are not authorized', HttpStatus.FORBIDDEN);
    }

    const [totalUsers, activeUsers, totalWorkspaces, userCountsPerWorkspace, usersLoggedInLast24Hrs] = await Promise.all([
      this.usersDatabase.getTottalUsers(),
      this.usersDatabase.getTottalActiveUsers(),
