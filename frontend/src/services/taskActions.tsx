import { Project, ProjectPriority } from "@/storage/redux/projectsSlice";
import { deleteFromLocalStorage, getLocalStorage, setLocalStorage } from "@/storage/storage";
import { message } from "antd";
import { PriorityDto, Session, StatusDto, TaskDto } from "models/tasks";
import {
    formatDate,
    getFormattedTime,
...
export const updateTask = (task: any, taskName: string) => {
    try {
        const taskList = getLocalStorage("TaskList").filter((tn: string) => tn !== taskName);
        if (taskList.includes(task.name)) {
            ...
            console.log("🚀 ~ file: taskActions.tsx:19 ~ updateTask ~ taskList", taskList);
            if (task?.totalTime >= 0) {
...
export function sortByStatus(tasks: TaskDto[]): TaskDto[] {
    ...
    return tasks.sort((a, b) => order[a.statusCategoryName] - order[b.statusCategoryName]);
}
...
export const getProjectStatusList = (projects: Project[], projectId: number) => {
    if (!projects) return [];
...
export const getPriorityList = (projects: Project[], projectId: number) => {
    if (!projects) return [];
    for (const project of projects) {
        if (project.id === projectId) {
            const tmpArray: any = project.priorities?.map((priority) => {
                return {
                    name: priority.name,
                    statusCategoryName: priority.priorityCategoryName as PriorityDto,
                };
            });
            return tmpArray;
        }
    ...
    const tmpTasks = tasks.map((task: TaskDto) => {
        ...
        const ended =
            task.sessions && !checkIfRunningTask(task.sessions)
                ? getFormattedTime(formatDate(task.sessions[task.sessions?.length - 1]?.endTime))
                : task.sessions[0]
                ? "Running"
                : "Not Started";
        ...
        return {
            ...
            percentage: task.estimation
                ? Math.round(getTotalSpentTime(task.sessions) / (task.estimation * 36000))
                : -1,
