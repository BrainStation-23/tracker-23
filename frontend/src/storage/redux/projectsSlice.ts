...
import { PriorityDto, StatusDto } from "models/tasks";
...
function isSame(status: StatusType, ar: StatusType) {
    return status.name === ar.name && status.statusCategoryName === ar.statusCategoryName;
}
        ...
        setProjectsSlice: (state, action: PayloadAction<Project[]>) => {
            ...
            action.payload.forEach((project: Project) => {
                project.statuses?.forEach((status: ProjectStatus) => {
                    ...
                    if (!tmpArray.find((item) => isSame(tmpStatus, item))) tmpArray.push(tmpStatus);
                });
            });
...
export const { setProjectsSlice, deleteProjectsSlice, resetProjectsSlice } = projectsSlice.actions;
